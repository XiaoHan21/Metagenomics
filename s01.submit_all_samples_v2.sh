#!/bin/bash
# ===============================================
#  Submit all metagenome samples via PBS job array
#  - Automatically generate a job array
#  - Max simultaneous jobs = n
# ===============================================

core_pipeline="/home/han_xiao/xiaohan/Others/Kangxing/09.FreshSpeak/01.metagenomics/02.script/run_metagenome_pipeline.sh"

RAW_DIR="/home/jason.kangx/Database/2026_FRESH-SPEAK/Metagenomics/"
OUT_DIR="/home/han_xiao/xiaohan/Others/Kangxing/09.FreshSpeak/01.metagenomics/03.result/"
PIPELINE=${1:-kraken}   # options: kraken or metaphlan
THREADS=${2:-32}

MAX_JOB_NUM=5

mkdir -p $OUT_DIR

# 收集所有样本名
mapfile -t SAMPLES < <(ls "$RAW_DIR")
NUM=${#SAMPLES[@]}

# 生成 job array 脚本
ARRAY_JOB="run_metagenome_array.sh"

cat > "$ARRAY_JOB" <<EOF
#!/bin/bash
#PBS -N meta_array_${PIPELINE}
#PBS -P as_lkc_sunny.wong
#PBS -l select=1:ncpus=${THREADS}:mem=100g
#PBS -l walltime=240:00:00
#PBS -q qintel_wfly
#PBS -J 1-${NUM}%${MAX_JOB_NUM}
#PBS -e ${OUT_DIR}/array_job.e
#PBS -o ${OUT_DIR}/array_job.o

# 加载样本数组
SAMPLES=(${SAMPLES[@]})

# 当前 job 对应的样本（PBS_ARRAY_INDEX 从 1 开始）
SAMPLE=\${SAMPLES[\$((PBS_ARRAY_INDEX - 1))]}

cd ${OUT_DIR}

echo "Running sample: \$SAMPLE"
bash ${core_pipeline} "${RAW_DIR}/\${SAMPLE}" "${PIPELINE}" "${OUT_DIR}" "${THREADS}"
EOF

# 提交 array job
echo "Submitting job array with ${NUM} samples (max ${MAX_JOB_NUM} concurrent)..."
qsub "$ARRAY_JOB"
